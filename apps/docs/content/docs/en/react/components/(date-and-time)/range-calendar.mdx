...
| `isDateUnavailable` | `(date: DateValue, anchorDate: CalendarDate \| null) => boolean` | - | Marks dates as unavailable. If `archorDate` is set, it will be used to adjust the available dates based on the user's first selected date. |
