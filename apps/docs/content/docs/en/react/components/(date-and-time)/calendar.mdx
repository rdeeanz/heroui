...
| `visibleDuration` | `{months?: number; weeks?: number; days?: number}` | `{months: 1}` | Visible time range. Month view is fully styled; week/day views are supported upstream but not optimized in HeroUI styles yet. |
