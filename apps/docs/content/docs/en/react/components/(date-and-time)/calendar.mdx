...
| `visibleDuration` | `{months?: number; weeks?: number; days?: number}` | `{months: 1}` | Visible time range. Use `{ weeks: n }` for week view or `{ months: n }` for month view. Day view is supported upstream but not optimized in HeroUI styles yet. |
