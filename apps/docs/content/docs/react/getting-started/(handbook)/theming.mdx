...
|------|------------------|----------|
| HeroUI (default) | *(unset)* | Thin thumb from theme tokens |
...
**Custom overflow areas** — use the `scrollbar`, `scrollbar-default`, or `scrollbar-none` utilities from `@heroui/styles` on your own elements. See [Styling](/docs/handbook/styling) for class-based overrides.
